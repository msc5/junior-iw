# NOT MY CODE
# Pytorch Implementation of Conv LSTM
# Source: https://github.com/ndrplz/ConvLSTM_pytorch/blob/master/convlstm.py
# Source: https://holmdk.github.io/2020/04/02/video_prediction.html
...
class EncoderDecoderConvLSTM(nn.Module):
    ...
    def autoencoder(
        ...
        # encoder
        for t in range(seq_len):
            h_t, c_t = self.encoder_1_convlstm(input_tensor=x[:, t, :, :], cur_state=[
                                               h_t, c_t])  # we could concat to provide skip conn here
            h_t2, c_t2 = self.encoder_2_convlstm(
                input_tensor=h_t, cur_state=[
...
if __name__ == '__main__':
    ...
    model = EncoderDecoderConvLSTM(64, 1).to(device)
    summary(model, input_size=(1, 10, 1, 64, 64))

    x = torch.rand((1, 10, 1, 64, 64)).to(device)
    output = model(x, 1)
