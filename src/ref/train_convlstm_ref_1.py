...
from conv_lstm_ref_1 import EncoderDecoderConvLSTM
...
class MovingMNISTLightning(pl.LightningModule):
    ...
    def create_video(self, x, y_hat, y):
        ...
        difference_plot = torch.cat([zeros.cpu().unsqueeze(
            0), difference.unsqueeze(0).cpu()], dim=1)[0].unsqueeze(1)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.log_images:
            if self.global_step % 250 == 0:
                ...
                self.logger.experiment.add_image('epoch_' +
                                                 str(self.current_epoch) +
                                                 '_step' +
                                                 str(self.global_step) +
                                                 '_generated_images', final_image, 0)
                plt.close()
    ...
    def configure_optimizers(self):
        return torch.optim.Adam(
            self.parameters(), lr=opt.lr, betas=(
                opt.beta_1, opt.beta_2))
