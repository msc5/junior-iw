# Copyright 2020 Mickael Chen, Edouard Delasalles, Jean-Yves Franceschi,
...
# Stolen from https://github.com/edouardelasalles/srvp/blob/master/data/kth.py
...
class KTH(VideoDataset):
    ...
    def __getitem__(self, index):
        ...
        # height)
        return torch.from_numpy(x).unsqueeze(3).float()
        ...
        if train:
            ...
            for c in cls.classes:
                for vid in os.listdir(join(data_dir, c)):
                    ...
                    images = sorted([join(data_dir, c, vid, img) for img in os.listdir(
                        join(data_dir, c, vid)) if os.path.splitext(img)[1] == '.png'])
                    data.append(images)
        else:
