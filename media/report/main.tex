...
of an RNN is given below in the form of a discrete Delay Difference Equation
(DDE) \cite{rnn_and_lstm_fundamentals}:

\begin{equation}
	\begin{split}
		\vec{s} [n] & = W_s \vec{s} [n - 1] + W_r \vec{r} [n - 1] + W_x \vec{x} [n] + \vec{\theta_s} \\
		\vec{r} [n] & = G ( \vec{s} [n] )
	\end{split}
...
however, it can be written more succinctly in the following form, ignoring the
cell's signal from memory $\vec{s} [n - 1]$, since its affect on the signal's
trajectory is negligible as long as $\vec{r} [n - 1]$ is still present:

\begin{equation}
	\begin{split}
		\vec{s} [n] & = W_r \vec{r} [n - 1] + W_x \vec{x} [n] + \vec{\theta_s} \\
		\vec{r} [n] & = G ( \vec{s} [n] )
	\end{split}
	\label{eq:rnn_standard}
\end{equation}

A more understandable depiction of the RNN described in Equation
\ref{eq:rnn_standard} can be found in Figure \ref{fig:rnn_arch}. This figure
...
$\vec{x} [t]$, and ouput signal $\vec{r} [t]$ (now given by $\vec{y} [t]$)
as they are produced over time.
